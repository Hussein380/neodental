      ...
      >
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-5 font-semibold">
              <a
                ...
                className="flex items-center gap-1.5 text-white hover:text-neo-clinical-light transition-colors"
              >
        ...
        <div
          className={`transition-all duration-300 ${
            isScrolled
              ? "bg-white shadow-sm border-b border-neo-clinical/15 py-3"
        ...
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                ...
                if (link.hasDropdown) {
                  return (
                    ...
                    >
                      <Link
                        ...
                        className={`inline-flex items-center gap-1 text-sm font-bold transition-colors py-2 ${
                          pathname.startsWith("/treatments")
                            ? "text-neo-red"
                            : "text-neo-navy hover:text-neo-red"
                        }`}
                      >
                        <span>{link.name}</span>
                        <ChevronDown
                      ...
                      {/* Dropdown Menu */}
                      {isTreatmentsDropdownOpen && (
                        <div className="absolute top-full left-0 w-80 bg-white rounded-2xl shadow-xl border border-neo-clinical/20 p-3 grid grid-cols-1 gap-1 animate-fade-in z-50">
                          <div className="px-3 py-1.5 text-[11px] font-bold text-neo-blue-gray uppercase tracking-wider border-b border-slate-100 mb-1 flex items-center justify-between">
                            <span>Dental Services</span>
                            <span className="text-[10px] text-neo-clinical">
                              Eastleigh
                            </span>
                          </div>
                          {treatmentCategories.map((cat) => (
                            <Link
                              ...
                              className="px-3 py-2 rounded-xl text-xs font-semibold text-neo-navy hover:bg-neo-ice hover:text-neo-red transition-all flex items-center justify-between"
                            >
                              <span>{cat.name}</span>
                              {cat.badge && (
            ...
            <div className="hidden lg:flex items-center gap-4">
              {/* Language Selector */}
              <div className="flex items-center bg-neo-ice p-1 rounded-full border border-neo-clinical/20 text-xs font-bold">
                <Globe className="w-3.5 h-3.5 text-neo-blue-gray ml-1.5 mr-1" />
                {languages.map((l) => (
                  <button
                    key={l.code}
                    type="button"
                    ...
                    className={`px-2.5 py-0.5 rounded-full transition-all text-xs font-bold ${
                      language === l.code
                        ...
                        : "text-neo-blue-gray hover:text-neo-navy"
                    }`}
                  >
                    {l.label}
                  </button>
            ...
            <div className="flex items-center gap-2 lg:hidden">
              <div className="flex items-center bg-neo-ice p-0.5 rounded-lg border border-neo-clinical/20 text-[10px] font-bold">
                {languages.map((l) => (
                  <button
                    key={l.code}
                    type="button"
              ...
              <button
                ...
                className="p-2 rounded-xl text-neo-navy hover:bg-neo-ice transition-colors border border-neo-clinical/20"
