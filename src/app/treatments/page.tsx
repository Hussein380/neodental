...
export default function TreatmentsHubPage() {
  ...
  return (
    <div className="pt-28 pb-24 bg-slate-50 min-h-screen">
      ...
      {/* 2. Interactive Symptom-to-Treatment Finder (Clean Clinical Theme) */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm relative overflow-hidden">
          {/* Subtle Accent Stripe */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-neo-red via-red-400 to-neo-clinical" />
          ...
          <div>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <h3 className="text-xl sm:text-2xl font-extrabold text-neo-navy">
                  Not sure what you need? <span className="text-neo-red">Select your symptom:</span>
                </h3>
                <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-xl">
                  Click any common condition below to instantly find and highlight the recommended dental treatment.
                </p>
              </div>
              ...
              {activeSymptom && (
                <button
                  ...
                  className="self-start md:self-auto inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold transition-colors"
                >
              ...
              {interactiveSymptoms.map((sym) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-start gap-3 p-3.5 rounded-2xl text-left transition-all duration-200 border ${
                      isSelected
                        ? "bg-red-50/70 border-red-500 shadow-sm ring-2 ring-red-400/30 scale-[1.01]"
                        : "bg-slate-50/70 hover:bg-white hover:border-slate-300 border-slate-200/80 hover:shadow-xs"
                    }`}
                  >
                    <span className="text-xl sm:text-2xl flex-shrink-0 mt-0.5">{sym.emoji}</span>
                    <div className="min-w-0">
                      <p className={`text-xs sm:text-sm font-bold truncate ${isSelected ? "text-neo-red" : "text-neo-navy"}`}>
                        {sym.label}
                      </p>
